...
class Permutation:
    def ConstructRandomOracle(size: int):
        ...
        oracle = np.random.randint(low=0, high=rmax, size=size)
        while len(set(oracle)) < len(oracle):
            oracle = np.random.randint(low=0, high=rmax, size=size)
        return oracle
...
class BuggyRootProtocol2:
    ...
    def initialize_memory(self, size: int):
        ...
        self.block_list = list(range(size)) + [-1] * self.square_root
    ...
    # step 1
    def scan_shelter(self, bID, curr_counter):
        shelter_start_idx = self.permuted_memory_size
        shelter_end_idx = self.real_memory_size - 1
        curr_pointer = shelter_start_idx
        is_found = False
    ...
    def cache_retrieved_data_to_shelter(self, bID):
        ...
        while curr_pointer <= shelter_end_idx:
            ...
            if mem_unit[0] == MemoryUnitType.NonEmpty:
                if successful_write:
                    self.real_memory.write_nothing(curr_pointer)
                elif mem_unit[1]["blockID"] == bID:
                    self.real_memory.write(curr_pointer, bID)
                    successful_write = True
            elif mem_unit[0] == MemoryUnitType.Empty and not successful_write:
                self.real_memory.write(curr_pointer, bID)
                successful_write = True
            curr_pointer += 1
...
class VirtualMemory:
    def __init__(self, size):
        self.protocol = BuggyRootProtocol2(size)
        self.size = size
        self.square_root = int(np.sqrt(size))
        self.counter = 0
    ...
    def write(self, bID):
        ...
        if is_found:
            # buggy part
            self.protocol.retrieve_from_permuted_memory(self.square_root+self.counter)
            self.protocol.cache_retrieved_data_to_shelter(-1)
        else:
            try:
                addr, mem_data = self.protocol.retrieve_from_permuted_memory(bID)
