...
def construct_hash_func(input_size, output_size):
    hash_func = [crypto_random.randint(0, output_size-1) for i in range(input_size)]
    return hash_func

class HierarchicalProtocol:
    def __init__(self, size):
        self.virtual_memory_size = size
        self.bucket_size = int(np.ceil(np.log2(size)))
    ...
    def get_bucket_range(self, level_index, bucket_index):
        ...
        return level_start + bucket_index * self.bucket_size, level_start + (bucket_index + 1) * self.bucket_size
    ...
    def retrive_higher_level_dummy(self, level, found_bid):
        # found_bucket = self.hash_funcs[level][found_bid]
        # bucket_list = [i for i in range(2 ** (level+1))]
        # prob_dist = [1/(2 ** (level+1)) for i in range(2 ** (level+1))]
        # prob_dist[found_bucket] -= 1/(self.bucket_size * 2 ** (level+1))
        # prob_dist = np.array(prob_dist)
        # prob_dist = prob_dist * (1/np.sum(prob_dist))
        # bucketID = np.random.choice(bucket_list, p=prob_dist)
        bucketID = np.random.randint(0, 2 ** (level+1))
        bucket_start, bucket_end = self.get_bucket_range(level, bucketID)
        for addr in range(bucket_start, bucket_end):
            self.real_memory.read(addr)
    ...
    def move_down(self, level):
        ...
        for addr in range(curr_start, next_end):
            mem_unit = self.real_memory.perfect_read(addr)
            if mem_unit[0] == MemoryUnitType.NonEmpty:
                blocks.add(mem_unit[1]["blockID"])
        self.clear_level(level)
        self.clear_level(next_level)
        blocks = list(blocks)
        no_collsion = False
        while not no_collsion:
            next_hash_func = construct_hash_func(self.virtual_memory_size, new_level_bucket_size)
            buck_bin = [next_hash_func[b] for b in blocks]
            no_collsion = len(buck_bin) == 0 or np.max(np.bincount(buck_bin)) <= self.bucket_size
            if no_collsion: 
                self.hash_funcs[next_level] = next_hash_func
        self.hash_funcs[level] = construct_hash_func(self.virtual_memory_size, 2 ** (level + 1))
        for block in blocks:
            next_hash_func = self.hash_funcs[next_level]
            bucketID = next_hash_func[block]
            bucket_start, bucket_end = self.get_bucket_range(next_level, bucketID)
            is_write = False
            for addr in range(bucket_start, bucket_end):
                mem_unit = self.real_memory.perfect_read(addr)
                if mem_unit[0] == MemoryUnitType.Empty and not is_write:
                    self.real_memory.perfect_write(addr, block)
                    is_write = True
            assert is_write            
        ...
        def random_bucket(bucket_bin):
            bucket_id = crypto_random.randint(0, new_level_bucket_size - 1)
            count = 0
...
class VirtualMemory:
    ...
    def read(self, bID:int):
        ...
        for level in range(1, self.protocol.level_num):
            if is_found:
                self.protocol.retrive_higher_level_dummy(level, bID)
            else:
    ...
    def reshuffle(self):
        ...
        #     # if level == 0 and self.req_num % (self.protocol.bucket_size) == 0:
        #     #     self.protocol.move_down(level)
        #     # elif level > 0 and self.req_num % (2 ** (level + 1)) == 0:
        #     #     self.protocol.move_down(level)
        #     if self.req_num % (2 ** (level + 1)) == 0:
        #         self.protocol.move_down(level)
        #     else:
